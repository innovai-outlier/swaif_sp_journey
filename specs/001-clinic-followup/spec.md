...
- Clinic Admin SHALL assign tasks to patients with due dates (direct assignment; templates optional).
...
- Deductions:
  - Done without delay/pause/reopen: 0
  - Done with delay: 20
  - Done with pause/reopen without delay: 25
  - Done with pause/reopen with delay: 40
  - Cancelled by patient: all remaining points (final 0)
  - Cancelled by clinic: no penalty; patient retains current points for that task

### FR-POINTS-02 Streak Bonus (Clinic Validated)
- Completion before due date is eligible for streak bonus.
...
- Reward fields include:
  ...
  - expires_at
  - eligibility rules (optional plan and/or pillar restriction)
...
- Admin SHALL see adherence segmentation based on quartiles and signals:
  - streak accumulation
  - delayed/paused/cancelled patterns
